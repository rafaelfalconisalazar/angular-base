...
export class HttpService {
  //static API_END_POINT = 'http://appssoluciones.com/tpvonline/factura-online/web/admin/';
  static API_END_POINT = 'http://127.0.0.1:8000/admin/';
  ...
  auth(endpoint: string, body?: Object): Observable<any> {
    return this.http.post(HttpService.API_END_POINT + endpoint, body, this.createOptions()).pipe(
      map(response => this.extractData(response)
      ), catchError(error => {
        return this.handleError(error);
  ...
  authToken(): HttpService {
    return this.header('Authorization', 'Bearer ' + localStorage.getItem('tokenadmin'));
  }
